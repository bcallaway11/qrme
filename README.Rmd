...
**`mcmc_draws` and `mcmc_burnin`** are the most consequential parameters.
Too few effective draws (= `mcmc_draws − mcmc_burnin`) per observation produces
...
**`nmix`** is selected by model fit rather than tuned. Use `logLik()`, `AIC()`,
and `BIC()` on fitted objects to compare across values; `nmix = 1` is the right

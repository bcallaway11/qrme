...
Quantile Regression with measurement error in the dependent variable
using an EM-algorithm. In practice, this function assumes that the
measurement error distribution is a mixture of normal distributions.
...
- se:
  ...
  Whether or not to compute standard errors using the bootstrap (default
  is FALSE)

- n_boot:

  Number of bootstrap iterations for standard errors (default 100)
...
an object of class "merr". Supports
